window.addEventListener('DOMContentLoaded', function() {
    ...
    const getResourse = async(url) => {
        ...
        if (!res.ok){
            throw new Error(`Couldn't  fetch status ${url}, status: ${res.status} `);
        }
    ...
    function currentSlideIndex() {
        return slides.length < 10? current.textContent = `0${slideIndex}`:current.textContent = slideIndex;
    }
    function currentSlideLength() {
        return slides.length < 10? total.textContent = `0${slides.length}`:total.textContent = slides.length;
    }
